...
# === PWM 변환 함수 (ADC 0~4095 → PWM 1250~1750) ===
...
# === LOITER 모드 조건 검사 후 ARM ===
def ensure_loiter_ready(vehicle):
    print("LOITER 모드 조건 확인 중...")
    while True:
        gps_status = vehicle.gps_0
        ekf_ok = getattr(vehicle, "ekf_ok", False)

        if gps_status.fix_type >= 3 and ekf_ok:
            print(f"GPS Fix={gps_status.fix_type}, EKF OK → 시동 준비 완료")
            break
        else:
            print(f"GPS Fix={gps_status.fix_type}, EKF={ekf_ok} → 대기 중...")
            time.sleep(1)

    print("LOITER 모드로 전환 중...")
    ...
    if not vehicle.armed:
        ...
        while not vehicle.armed:
            print("ARM 대기 중...")
            time.sleep(1)
        print("드론 시동 완료! LOITER 모드에서 제어 가능")
...
def handle_d3_toggle(d3, vehicle):
    ...
    if d3 is not None and last_d3 == 0 and d3 == 1:
        # --- 비행 중일 때: 착륙 ---
        if current_mode == "LOITER" and vehicle.armed:
            print("[D3] 착륙 명령 실행 중...")
    ...
    while True:
        try:
            if ser.in_waiting > 0:
                ...
                if vehicle.mode.name in ("LOITER", "ALT_HOLD"):
